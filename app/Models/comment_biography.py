...
class CommentBiography(db.Model):
    ...
    Attributes:
        ...
        comment_biography_content (str) : Contenu du commentaire.
        comment_biography_date (datetime) : Date et heure du commentaire.
        biography_mangaka_id (int) : Identifiant de la biographie associée au commentaire.
    ...
    comment_biography_content = db.Column(db.Text(), nullable=False)
    ...
    def __repr__(self):
        """
        Représentation de l'objet BiographyComment.

        Returns:
            str: Chaîne représentant l'objet BiographyComment.
        """
        return f"CommentBiography(id={self.id}, biography_mangaka_id={self.biography_mangaka_id}, " \
               f"user_id={self.user_id}, comment_date={self.comment_biography_date})"
